...
class TestVotingMachine < Minitest::Test
  ...
  def test_add_vote
    ...
    machine.record_vote(category1, "Pablo", "Maria")
    machine.record_vote(category1, "Suzie", "Maria")
    machine.record_vote(category1, "Samira", "Pablo")
    machine.record_vote(category2, "Juan", "Samira")
    ...
    assert_raises VotingMachine::InvalidCategory do
      machine.record_vote("Invalid category", "Sue", "Kapu")
    end
  end
end
